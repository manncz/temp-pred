...
```{r, child = c('00-wunderground-scrape.Rmd', '00-model-funs.Rmd','02-hist-model.Rmd'), include = F}
```
...
hist.all.pred <- foreach(i = 1:5, .combine = rbind) %do% {
  ...
  predict_temp_hist(current.temps= current.temp,
                    local = FALSE, tmin.model = hist.mods[[i]]$tmin,
                    tmax.model = hist.mods[[i]]$tmax, tavg.model = hist.mods[[i]]$tavg, 
                    days_lag = i)
